...
> = {
  teal: {
    selected:
      "border-[#30d8c3]/40 bg-[#143e3d] text-white",
    icon:
      "bg-[#4ce5d2] text-[#062521]",
  },
  violet: {
    selected:
      "border-[#a38cff]/40 bg-[#31295a] text-white",
    icon:
      "bg-[#b8a7ff] text-[#261d4c]",
  },
  amber: {
    selected:
      "border-[#ffc96f]/40 bg-[#553a18] text-white",
    icon:
      "bg-[#ffd583] text-[#432904]",
  },
};
...
export function PlaygroundWorkspace() {
  ...
  const selectedCourse =
    playgroundCourses.find(
      (course) =>
        course.id === selectedCourseId,
    ) ?? initialCourse;

  const selectedTopic =
    selectedCourse.topics.find(
      (topic) =>
        ...
        <div className="relative z-10 flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 max-w-2xl text-sm leading-7 text-white/60 sm:text-base">
              Resuelve ejercicios, solicita
              pistas y recibe retroalimentación
              inmediata sobre los temas de tus
              materias.
            </p>
          </div>
...
function PlaygroundSessionArea({
  ...
  const completedCount =
    selectedTopic.exercises.filter(
      (exercise) =>
      ...
      <aside className="space-y-5">
        <section className="rounded-[1.8rem] border border-primary/15 bg-secondary p-5">
          ...
          <p className="mt-5 text-sm leading-7 text-secondary-foreground/75">
            ...
            enunciado. Puedes solicitar
            pistas progresivas sin revelar
            directamente la respuesta.
          </p>

          <div className="mt-5 rounded-2xl border border-primary/15 bg-background/50 p-4">
            <div className="flex gap-3">
              ...
              <p className="text-xs leading-6 text-muted-foreground">
                ...
                palabras por qué seleccionaste
                una respuesta. Esto mejora la
