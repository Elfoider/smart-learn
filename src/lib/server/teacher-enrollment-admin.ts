...
function safeTimestampMillis(value: unknown) {
...
function safeStudentStatus(value: unknown): StudentAccountStatus {
...
function safeEnrollmentStatus(value: unknown): EnrollmentStatus {
  if (value === "inactive" || value === "completed") {
    return value;
  }
...
function parseEnrollment(id: string, data: DocumentData): TeacherEnrollment {
  return {
    id,
    teacherId: typeof data.teacherId === "string" ? data.teacherId : "",
    studentId: typeof data.studentId === "string" ? data.studentId : "",
    studentName:
      typeof data.studentName === "string" ? data.studentName : "Estudiante",
    studentEmail:
      typeof data.studentEmail === "string" ? data.studentEmail : "",
    studentPhotoURL:
      typeof data.studentPhotoURL === "string" ? data.studentPhotoURL : null,
    courseId: typeof data.courseId === "string" ? data.courseId : "",
    sectionId: typeof data.sectionId === "string" ? data.sectionId : "",
    status: safeEnrollmentStatus(data.status),
    enrolledAtMs: safeTimestampMillis(data.enrolledAt),
    updatedAtMs: safeTimestampMillis(data.updatedAt),
  };
}

function createEnrollmentId(courseId: string, studentId: string) {
  ...
  return snapshot.docs
    .map((studentDocument) => {
      ...
      return {
        ...
        name: typeof data.name === "string" ? data.name : "Estudiante",
        email: typeof data.email === "string" ? data.email : "",
        status: safeStudentStatus(data.status),
...
export async function listTeacherEnrollments(
  ...
  return snapshot.docs
    .map((enrollmentDocument) =>
      parseEnrollment(enrollmentDocument.id, enrollmentDocument.data()),
    )
...
export async function createTeacherEnrollment(
  ...
  const courseReference = db.collection("courses").doc(input.courseId);

  const sectionReference = db.collection("sections").doc(input.sectionId);

  const studentReference = db.collection("users").doc(input.studentId);

  const enrollmentId = createEnrollmentId(input.courseId, input.studentId);

  const enrollmentReference = db.collection("enrollments").doc(enrollmentId);
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const studentName =
      typeof student.name === "string" ? student.name : "Estudiante";

    const studentEmail = typeof student.email === "string" ? student.email : "";

    const studentPhotoURL =
      typeof student.photoURL === "string" ? student.photoURL : null;
    ...
    if (!enrollmentSnapshot.exists) {
      enrollmentData.enrolledAt = FieldValue.serverTimestamp();
    }

    transaction.set(enrollmentReference, enrollmentData, {
      merge: true,
    });
    ...
    return {
      ...
      sectionId: input.sectionId,
      status: "active",
      enrolledAtMs: enrollmentSnapshot.exists
        ? safeTimestampMillis(previousEnrollment?.enrolledAt)
        : now,
      updatedAtMs: now,
    };
  });
...
export async function updateTeacherEnrollment(
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const previousData = enrollmentSnapshot.data();

    if (!previousData) {
      throw new EnrollmentAdminError(
        "enrollment/invalid-data",
        "Los datos de la inscripción no están disponibles.",
        500,
      );
    }

    const previousEnrollment = parseEnrollment(
      enrollmentSnapshot.id,
      previousData,
    );

    if (previousEnrollment.teacherId !== teacherId) {
      throw new EnrollmentAdminError(
    ...
    const courseReference = db
      .collection("courses")
      .doc(previousEnrollment.courseId);

    const courseSnapshot = await transaction.get(courseReference);

    if (
      !courseSnapshot.exists ||
      courseSnapshot.data()?.teacherId !== teacherId
    ) {
      throw new EnrollmentAdminError(
        "enrollment/course-forbidden",
        "No puedes administrar esta asignatura.",
    ...
    const nextSectionId = input.sectionId ?? previousEnrollment.sectionId;
